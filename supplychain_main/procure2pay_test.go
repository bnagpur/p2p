...
var chaincodeName = "SupplychainChaincode"
...
func Test_Init(t *testing.T) {
	scc := new(SupplychainChaincode)
	stub := shim.NewMockStub("procure2pay", scc)
...
func Test_Invoke(t *testing.T) {
	...
	//stubEx2 := shim.NewMockStub(chaincodeName, ccEx2)
	checkInit(t, stub, [][]byte{[]byte("init")})
	stub.MockPeerChaincode(chaincodeName, stub)
	timestamp, _ := stub.GetTxTimestamp()

	checkInit(t, stub, [][]byte{[]byte("init")})

	// a + b = 222 + 333 = 555
	checkInvoke(t, stub, [][]byte{[]byte("addOrder") /* []byte(chaincodeName), []byte("processAddPurchaseOrder")*/, []byte(stub.GetTxID()), []byte("CISCO"), []byte("CISCO VOIP"), []byte("2"), []byte("450"), []byte("PO_STATUS_CREATED"), []byte(timestamp.String())})
